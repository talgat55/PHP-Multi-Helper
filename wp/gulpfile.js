...
gulp.task('sass', function () {
  return gulp.src('./wp-content/themes/light/static/sass/style.sass')
    .pipe(sass().on('error', sass.logError))
      .pipe(autoprefixer(['last 15 versions', '> 1%', 'ie 8', 'ie 7'], { cascade: true })) // Создаем префиксы
    .pipe(gulp.dest('./wp-content/themes/light/assets/css/'))
	.pipe(browserSync.reload({stream: true}));
});


gulp.task('update-file', function() { // Создаем таск browser-sync
    return gulp.src('./wp-content/themes/light/assets/css/*.css') // Берем источник
